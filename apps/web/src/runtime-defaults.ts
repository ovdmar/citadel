...
// else fall back to the first healthy non-shell runtime; "" when no agent
// runtime is available (only shell installed, or empty list).
//
...
export function defaultAgentRuntimeId(runtimes: ReadonlyArray<AgentRuntime>): string {
  const launchable = runtimes.filter((runtime) => runtime.id !== "shell" && runtime.health === "healthy");
