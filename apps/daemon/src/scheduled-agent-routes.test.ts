...
const INTEGRATION_TEST_TIMEOUT_MS = 45_000;

describe("scheduled agent routes", () => {
  it(
    "manages scheduled agents through CRUD and manual-run endpoints",
    async () => {
  ...
  it(
    "GET /runs returns the per-agent run rows; GET /log slices the log file; both 404 when the run doesn't belong to the agent",
    async () => {
      const fixture = createFixture();
      const git = createGitRepo(fixture.config.dataDir);
      const now = new Date().toISOString();
  ...
  it(
    "POST /run maps the four overlap-policy outcomes to HTTP envelopes (skip/queue/queue_full)",
    async () => {
      const fixture = createFixture();
      const git = createGitRepo(fixture.config.dataDir);
      const now = new Date().toISOString();
  ...
  it(
    "rejects invalid query parameters with 400",
    async () => {
      const fixture = createFixture();
      const git = createGitRepo(fixture.config.dataDir);
      fixture.store.insertRepo({
