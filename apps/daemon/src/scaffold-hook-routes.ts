...
export function buildHookScaffoldPrompt(input: { repo: Repo; template: string }): string {
  ...
  return [
    ...
    "",
    "2. If the repo has a stop command, also write `.citadel/hooks/undeploy`.",
    "   - It receives an optional app name in `$1`; no app name means undeploy all apps.",
    "   - It should stop the named app, or all apps if `$1` is empty.",
    "",
    "3. Make hook files executable: `chmod +x .citadel/hooks/deploy` and, if present,",
    "   `chmod +x .citadel/hooks/undeploy`.",
    "",
    "4. Validate: run `./.citadel/hooks/deploy list` and confirm it prints",
    `   \`{"apps":[{"name":"...","url":"http://..."}]}\` that parses as JSON.`,
    "",
    "5. Iterate until step 4 succeeds. Then stop — the operator will commit and open a PR through",
