...
describe("namespace routes + MCP integration", () => {
  it(
    "creates a namespace, creates a workspace in it, lists, and reassigns through the MCP tool surface",
    async () => {
  ...
  it(
    "rejects archiving an unknown namespace, supports restore + MCP includeArchived",
    async () => {
      const fixture = createFixture();
      const git = createGitRepo(fixture.config.dataDir);
      const now = new Date().toISOString();
      ...
      try {
        ...
        expect(
          mcpArchived.result.structuredContent.namespaces.find((entry) => entry.id === created.namespace.id)
            ?.archivedAt,
        ).not.toBeNull();
