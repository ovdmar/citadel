...
// Runtimes where tmux should own mouse wheel scrollback. Plain shells need
// this because tmux holds the pane history; some TUIs also grab DEC mouse
// tracking for in-app navigation. Claude Code is intentionally absent — it
// does not request mouse tracking, so its xterm-native wheel scroll stays
// untouched.
const MOUSE_GRABBING_RUNTIMES = new Set(["shell", "codex", "cursor-agent"]);
