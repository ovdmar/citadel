...
- **`make upgrade` verb.** Dedicated upgrade command. Defaults to the latest stable annotated release tag from `origin`; `make upgrade REF=main` explicitly installs latest `origin/main`; `make upgrade REF=v0.3.0` installs a specific annotated tag.
- **Pinned installs.** `make install REF=v0.3.0` checks out the requested annotated tag first. Refuses lightweight tags, arbitrary branches, SHAs, malformed/prerelease tags, and dirty trees.
