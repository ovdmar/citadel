...
[x] 8. Scrollback works for both plain shells and alt-screen TUIs. ttyd's xterm is the renderer and tmux owns the pane history. The shared citadel tmux server enforces a bounded global `history-limit`; ttyd attaches with session-scoped `mouse on`, `history-limit 50000`, and `set-clipboard on` for plain shell/codex/cursor-agent sessions so wheel events in a plain pane enter tmux copy-mode and selections reach the system clipboard via OSC 52. Claude Code keeps xterm-native wheel behavior and continues to handle its own scrollback.
