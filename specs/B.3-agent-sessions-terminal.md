...
[~] 4. The primary browser terminal renderer is Citadel's in-process xterm.js cockpit pane connected to the daemon WebSocket at `/terminal/<sessionId>`. The daemon bridges that socket to the durable tmux session through tmux control mode, so normal workspace/session switching does not spawn per-session renderer processes.
[~] 5. Terminal input/output rides the daemon-owned WebSocket bridge and never goes to a 3rd-party host. `ttyd` remains available through `/terminals/<sessionId>/` as an explicit fallback/standalone renderer, but it is not spawned during normal cockpit navigation.
[ ] 6. App state, operations, events, and provider health use REST/SSE.
[~] 7. Citadel owns session creation, metadata, workspace/runtime association, tmux lifecycle, terminal permissions, WebSocket routing, tmux-control cleanup, and fallback ttyd spawn/cleanup.
[~] 8. Long terminal buffers stay responsive through bounded tmux scrollback and xterm.js' browser-side renderer. Reconnect sends only a bounded snapshot plus live incremental chunks for the active pane.
[~] 9. Fallback ttyd ports come from a configurable loopback-only range; the daemon scans for stale `ttyd` processes inside that range on startup and reaps them. At runtime the daemon also periodically detaches orphaned tmux clients (viewer state left behind by closed/crashed cockpit panels) and rotates the per-session pipe-pane log files in `${TMPDIR}/citadel-pty/`, so a long-running deployment does not accumulate per-client memory in the shared tmux server or unbounded log files on disk. The underlying agent session and any process running inside it are never affected — only the disconnected viewer. The pane process is the operator's login shell (`bash -l`); the agent runs as a child via `tmux send-keys`, so Ctrl+C inside the embedded terminal kills the agent without ending the pane.
[~] 10. Terminal pane state explains connecting, attached, reconnecting, error, and closed states. When the WebSocket bridge or fallback ttyd cannot attach (`session_not_found`, `tmux_session_missing`, `terminal_unavailable`, `ttyd_missing`, `no_free_port`, `ttyd_start_timeout`, `spawn_failed`) the cockpit shows an inline error with the code, detail, settings/runbook links, and a Retry button — never a blank black pane.
[~] 11. The Citadel daemon exposes the primary xterm/WebSocket gateway at `/terminal/:sessionId` for the cockpit, tooling, and tests.
[~] 12. Trade-offs of the xterm/WebSocket primary renderer are accepted: Citadel owns a small terminal bridge, but uses xterm.js and tmux control mode rather than a from-scratch terminal emulator. The benefit is snappy switching and low RAM because normal navigation no longer needs one external ttyd process and iframe per active session. ttyd remains as a compatibility fallback for terminal fidelity issues.
