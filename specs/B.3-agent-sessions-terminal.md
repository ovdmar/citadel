...
[~] 14. Workspace cards, stage tabs (per-agent), and the navigator "Running" stat all display the small pulsing lifecycle icon and follow the same five-tone `LifecycleTone` taxonomy: `never-started`, `running`, `done`, `rate-limited`, `attention`. CSS classes are reused from the shared `cit-pulse` / `cit-pulse-sm` chrome plus a new `cit-pulse-done` (green + ripple), a ripple addition to `cit-pulse-bad`, and `cit-pulse-info` (blue + ripple) for the rate-limited stall. Per-agent mapping (`deriveAgentLifecycleTone` in `@citadel/core`): `starting`/`running`/`idle` → `running` (active or between-turn waiting); `rate_limited` → `rate-limited`; `waiting_for_input` → `attention`; `stopped` with exit code in `{0, null, 130, 143}` → `done`; `stopped` with other non-zero exit → `attention`; `failed` → `attention`; `unknown` → `attention` when `sessionNeedsAttention()` is true (tmux-gone reasons), else `running` (indeterminate but possibly alive). Per-agent never returns `never-started`. Workspace mapping (`deriveWorkspaceLifecycleTone` in `@citadel/core`): filter out shell sessions; if none remain → `never-started`; else aggregate by priority `attention > rate-limited > running > done`; then fold PR/CI — if any check in `pullRequest.checks` has a conclusion in the failure set (`failure`, `cancelled`, `timed_out`, `action_required`), escalate to `attention` regardless of agent aggregate. This is the same conclusion set `prToneFor` keys on in `apps/web/src/workspace-card.tsx:443-459`, but the core helper inlines the check directly (rather than delegating to the web-layer `prToneFor`) so the layering stays one-way — apps/web depends on core, never the other direction. CI-red wins over a running agent because the failing CI is the more actionable operator signal. CSS class mapping (`lifecycleToneClass` in `apps/web/src/workspace-card.tsx`): `never-started → cit-pulse-idle`, `running → cit-pulse-run`, `done → cit-pulse-done`, `rate-limited → cit-pulse-info`, `attention → cit-pulse-bad`. `cit-pulse-ok` (solid green) is reserved for non-lifecycle uses (auto-mode pill, deploy-health badge) and is not part of this taxonomy. The navigator receives a `workspacePullRequests` map from the cockpit so per-workspace PR data participates in the aggregate.
