...
[ ] 14. Workspace cards display a small pulsing status icon before the workspace name, aggregated across the workspace's agent sessions: green (pulsing) when at least one agent is `running`; red (pulsing, attention) when at least one is `waiting_for_input`, `failed`, or `unknown` with a tmux-gone reason; grey (static) otherwise. Priority red > green > grey.
