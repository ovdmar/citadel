...
[x] 11. A compact bottom status bar renders machine health indicators on the left (overall health, CPU, RAM, disk usage/free space, disk I/O load when available, and Citadel process RSS) and keeps only an actual live-watch label plus the clock on the right. Health values stream over SSE every 5s while a cockpit viewer is connected, with a slower HTTP poll as fallback.
...
[x] 1. Global cockpit shortcuts:
   ...
   - `Shift+Cmd+D` (Mac) / `Shift+Ctrl+D` (other) — starts browser voice dictation when the browser delivers the key event and the focused target can accept text. If no editable target is focused but the cockpit has a selected active terminal session, Citadel focuses that session and targets it. Dictation uses the Web Speech API with `en-US`, snapshots the resolved target at start, treats recognized speech as literal text, and auto-commits after the configurable silence window (15 seconds by default). The voice overlay also exposes a manual send action for the current draft. If Web Speech is unavailable or `start()` is rejected after the chord is delivered, Citadel shows a retry/unavailable state instead of falling back to scratchpad.
...
[ ] 5. A pinned composer at the bottom of the list is always visible. Cmd/Ctrl-Enter or blur-with-non-empty-content creates a new block at the end of the file, except that focus moves into composer action controls or voice overlay controls are treated as part of the composer interaction and preserve the draft. The list autoscrolls so the composer stays in view.
...
[x] 18. On mobile-sized bare-root launch, Citadel opens the existing scratchpad drawer by rewriting `/` to `/?scratchpad=1` before last-route restoration. Search/hash deeplinks and non-root routes are preserved, and the drawer focuses the composer where browser focus policy permits. The composer exposes a mic button when browser speech recognition is available; tapping it starts the same global voice engine targeted at the composer. Auto-submit is on by default and creates a block after the configured silence window (15 seconds by default), while the voice overlay can manually send the current draft. No separate mobile scratchpad page is introduced in v1.
[x] 19. Global voice dictation targets the focused input surface, not only scratchpad. Valid v1 targets are registered Citadel edit surfaces, `textarea` controls, text-like non-sensitive `input` controls (`text`, `search`, `email`, `tel`, `url`), focused terminal panes, and the cockpit-selected active terminal session when no editable target is focused. If no valid target exists, the transcript remains in a copyable voice overlay and is never silently written to scratchpad.
