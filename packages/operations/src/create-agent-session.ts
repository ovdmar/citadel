    ...
    if (!isShellRuntime) {
      ...
      runtimeLaunchStartedMs =
        input.runtimeId === "codex"
          ? await withCodexLaunchLock(() => launchCodexWithRetry(sessionName, runtime.command, runtimeArgs, exitHint))
          : await launchRuntimeOnce(sessionName, runtime.command, runtimeArgs, exitHint);
    }
    if (promptForKeys) {
...
async function launchRuntimeOnce(
  ...
  await launchAgentInSession(sessionName, command, args, { exitHint });
  return startedAt;
}
