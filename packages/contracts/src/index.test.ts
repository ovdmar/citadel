...
describe("contract schemas", () => {
  ...
  it("validates the canonical AgentSessionStatus enum and round-trips limited statuses", () => {
    const allValues = [
      "starting",
      "running",
      "waiting_for_input",
    ...
    expect(rl.statusReason).toBe("pane:rate_limited:server");
  });
  ...
  it("typed status helpers partition the canonical enum correctly", () => {
    ...
    expect(isInteractiveStatus("usage_limited")).toBe(false);
    ...
    expect(isAliveStatus("usage_limited")).toBe(true);
    expect(isAliveStatus("unknown")).toBe(true);
    expect(isAliveStatus("stopped")).toBe(false);
    expect(isAliveStatus("failed")).toBe(false);
    // isAcceptingInputStatus mirrors sendAgentMessage's shell-first gate:
    // limited statuses can accept a resume nudge, but idle means shell prompt.
    ...
    expect(isAcceptingInputStatus("idle")).toBe(false);
    expect(isAcceptingInputStatus("rate_limited")).toBe(true);
    expect(isAcceptingInputStatus("usage_limited")).toBe(true);
