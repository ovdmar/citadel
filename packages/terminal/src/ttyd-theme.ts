...
export function resolveTtydThemeFromJson(themeJson: string | null): TtydTheme {
  if (themeJson?.includes(`"${LIGHT_XTERM_THEME.background}"`)) return "light";
  return "dark";
}

// Palette matches the cockpit's warm-cream redesign so the terminal pane reads
// as part of the surface. White values are remapped on the light theme so
// explicit white terminal output stays visible on cream.
