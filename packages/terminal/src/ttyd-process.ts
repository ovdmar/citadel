...
const LIGHT_XTERM_BACKGROUND = "#f5f1e8";
const DARK_XTERM_BACKGROUND = "#1a1814";
...
function readTtydEntryFromProc(pid: number, port: number, basePathPrefix: string): TtydEntry | null {
  ...
  if (themeJson) {
    if (themeJson.includes(`"${LIGHT_XTERM_BACKGROUND}"`)) theme = "light";
    else if (themeJson.includes(`"${DARK_XTERM_BACKGROUND}"`)) theme = "dark";
  }
