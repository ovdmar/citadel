...
import { reviewMcpToolDefinitions } from "./review-tools.js";
...
export function callMcpTool(call: McpToolCall, context: McpToolContext) {
  switch (call.name) {
    ...
    case "delete_review_comment":
      // Review tools require the daemon for either fs access (DB writes) or
      // hook execution. The in-process path holds no daemon handle, so we
      // route to the daemon-side dispatcher.
