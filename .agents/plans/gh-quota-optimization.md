...
- For every distinct repo in `store.listRepos()`, run `git ls-remote <primaryRemote> refs/heads/<defaultBranch>` from `repo.rootPath`. Resolves `primaryRemote` from the first entry of any workspace's `remotes[]` for that repo (matches `currentPullRequest`'s remote-selection semantics — handles `upstream`-style fork workflows). Falls back to first available workspace path with a warn log if `rootPath` is missing or unreadable.
